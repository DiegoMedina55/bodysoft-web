...
const LoginForm = (props) => {
  ...
  const [authentication, { data, loading, error }] = useMutation(
    AUTHENTICATION,
    { errorPolicy: "all" }
  );
  const [
    getTokenData,
    { called, data: tokenData, loading: tokenLoading, error: tokenError },
  ] = useMutation(AUTHTOKEN, { errorPolicy: "all" });

  const callTokenData = async () => {
    try {
      await getTokenData({
        variables: { Token: data.webAuthentication.Token },
      });
    } catch (e) {}
  };
  ...
  if (tokenData) {
    ...
    localStorage.setItem(
      "profile",
      tokenData.webValidateAuthToken.Profile ? "true" : ""
    );
  }
  ...
  return (
    <>
      ...
      <button
        type="button"
        className="btn btn-link FontBlackLink"
        onClick={() => {
          props.changePassHandler();
        }}
      >
